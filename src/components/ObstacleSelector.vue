...
<template>
  <div class="flex flex-col gap-2.5">
    <div
      v-for="(row, rowIndex) in layoutConfig"
      :key="rowIndex"
    ...
    >
      <template v-for="(id, colIndex) in row" :key="id || `empty-${rowIndex}-${colIndex}`">
        <div v-if="id" class="relative">
          <!-- 障碍物主选择按钮 -->
          <button
            @click="selectObstacle(id)"
            class="group relative flex items-center justify-center rounded-xl p-2 transition-all duration-200"
            :class="[
          ...
          >
            <div class="relative flex max-w-20 items-center justify-center">
              <img
                :src="getObstacleImageUrl(id)"
                :alt="`Obstacle ${id}`"
                class="pointer-events-none h-full w-full object-contain transition-transform duration-200 group-hover:scale-105"
              />
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <div class="mb-3">
                ...
                <div class="grid max-h-60 grid-cols-2 gap-1.5 overflow-y-auto p-0.5">
                  <button
                    v-for="item in defaultGlobalConfig.randomType"
                    :key="item"
                    @click="toggleRandomType(item)"
                    class="group flex h-10 w-full shrink-0 items-center justify-center rounded-lg border p-1 transition-all active:scale-95"
                    :class="[
                  ...
                  >
                    <img
                      :src="getObstacleImageUrl(String(item))"
                      :alt="`Obstacle ${item}`"
                ...
                <div class="mb-2 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1">
                    <input
                      ...
                      min="0"
                      max="100"
                      v-model.number="globalConfigStore.repeatChance"
                      class="w-12 [appearance:textfield] rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-right font-mono text-xs font-semibold text-slate-700 transition-all focus:border-slate-400 focus:bg-white focus:outline-hidden [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                    />
        ...
        <div
          v-else
          class="pointer-events-none invisible flex items-center justify-center rounded-xl border border-transparent p-2"
          aria-hidden="true"
        >
          <div class="relative flex max-w-20 items-center justify-center">
            <img
              v-if="getColumnSampleId(colIndex)"
              :src="getObstacleImageUrl(getColumnSampleId(colIndex)!)"
