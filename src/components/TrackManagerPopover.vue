<template>
  ...
  >
    ...
    <!-- 弹窗内部面板 -->
    <div
      class="flex flex-col gap-3 overflow-hidden rounded-xl bg-white p-3.5 text-slate-800"
    ...
    >
      ...
      <!-- 音轨列表 -->
      <div class="flex max-h-56 flex-col gap-2 overflow-x-hidden overflow-y-auto pr-2.5">
        <!-- 主音轨：默认 100% -->
        <div
          v-if="appStore.currentSong?.audioFile"
          class="flex w-full min-w-0 items-center justify-between gap-2 rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs"
        >
          <div class="flex min-w-0 flex-1 items-center gap-1.5">
            <span
              class="shrink-0 rounded bg-indigo-100 px-1 py-0.5 text-[10px] font-bold text-indigo-600"
              >主音轨</span
            >
            <span
              class="min-w-0 truncate font-medium text-slate-700"
              :title="appStore.currentSong.audioFile.name"
            >
          ...
          <div class="flex shrink-0 items-center gap-1">
            ...
            <button
              type="button"
              class="mr-1 flex h-6 w-6 items-center justify-center rounded text-slate-400 transition-colors hover:bg-slate-200/60 hover:text-slate-700"
              :title="localMainVolume === 0 ? '取消静音' : '静音'"
              @click="toggleMainMute"
            >
              <svg
                v-if="localMainVolume === 0"
        ...
        <template v-if="appStore.currentSong?.backingTracks?.length">
          <div
            v-for="(track, index) in appStore.currentSong.backingTracks"
            :key="index"
            class="flex w-full min-w-0 items-center justify-between gap-2 rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs"
          >
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span
                class="shrink-0 rounded bg-slate-200 px-1 py-0.5 text-[10px] font-bold text-slate-600"
                >副音轨</span
              >
              <span class="min-w-0 truncate font-medium text-slate-700" :title="track.name">
                {{ track.name }}
              </span>
            </div>
            ...
            <div class="flex shrink-0 items-center gap-1">
              ...
              <button
                type="button"
                class="flex h-6 w-6 items-center justify-center rounded text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-600"
                title="删除音轨"
                @click="removeBackingTrack(index)"
              >
                <svg
                  class="h-3.5 w-3.5"
                  fill="none"
              ...
              <button
                type="button"
                class="flex h-6 w-6 items-center justify-center rounded text-slate-400 transition-colors hover:bg-slate-200/60 hover:text-slate-700"
                :title="localBackingVolumes[index] === 0 ? '取消静音' : '静音'"
                @click="toggleBackingMute(index)"
              >
                <svg
                  v-if="localBackingVolumes[index] === 0"
                  class="h-3.5 w-3.5 text-rose-500"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
              ...
              <div class="w-16">
                <el-slider
                  v-model="localBackingVolumes[index]"
                  :min="0"
                  :max="1"
                  :step="0.01"
                  size="small"
        ...
        <!-- 无音轨提示(需要吗？？？确信) -->
        <div
          v-if="!appStore.currentSong?.audioFile && !appStore.currentSong?.backingTracks?.length"
